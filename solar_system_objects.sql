/*This project contains data sets of solar system objects and helps to learn use case of, as follows
1.) average, max, and calculations.
2.) using HAVING.
3.) using CASE.
4.) using AND/OR 
...
SELECT AVG(mass) AS average_mass,MAX(mass) AS max_mass, MIN(mass) AS min_mass  FROM solar_system_objects ;
SELECT * FROM solar_system_objects WHERE body IN('Mercury', 'Mars', 'Jupiter', 'Neptune', 'Saturn', 'Sun', 'Uranus', 'Venus') GROUP BY body HAVING volume > 50000;
SELECT COUNT(*), CASE 
WHEN mean_radius < 6000 THEN "smaller than earth"
WHEN mean_radius > 6000 AND mean_radius < 7000 THEN "approximate to earth"
WHEN mean_radius > 7000 THEN "larger than earth"
END AS compared_with_earth FROM solar_system_objects GROUP BY compared_with_earth;
